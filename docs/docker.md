...
- **`dev` 與 `dev-gpu` 二選一**：兩個 profile 各對應一個開發後端（`backend-dev-cpu` / `backend-dev-gpu`），皆綁定主機 `8000`；請勿同時啟用兩個 profile，否則埠衝突。
- **dev / dev-gpu**（擇一）：後端 `http://localhost:8000`，前端 Vite `http://localhost:5173`。前端透過 `VITE_API_TARGET=http://api:8000` 將 `/api`、`/health` 轉到後端（Compose 網路別名 `api`）。
